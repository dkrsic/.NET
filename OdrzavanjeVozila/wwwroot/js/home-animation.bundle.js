(() => {
  ...
  // frontend/home-animation.js
  var [$container] = utils_exports.$(".container");
  var [$range] = utils_exports.$(".range");
  for (let i = 0; i < 150; i++) {
    const $particle = document.createElement("div");
    $particle.classList.add("particle");
    $container.appendChild($particle);
    animate($particle, {
      x: utils_exports.random(-10, 10, 2) + "rem",
      y: utils_exports.random(-3, 3, 2) + "rem",
      scale: [{ from: 0, to: 1 }, { to: 0 }],
      delay: utils_exports.random(0, 1e3),
      loop: true
    });
  }
  function onInput() {
